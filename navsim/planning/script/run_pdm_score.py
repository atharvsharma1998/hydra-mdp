...
def main(cfg: DictConfig) -> None:
    ...
    score_cols = [
        ...
        if (
            ...
            and c not in ["pdm_score", "weighted_metrics", "weighted_metrics_array"]
        )
    ]

    if not pseudo_closed_loop_valid:
        mean_scores = pdm_score_df[score_cols].mean()
        pcl_stage1_score = mean_scores
        pcl_stage2_score = pd.Series(index=score_cols, dtype=object)
        pcl_group_score = mean_scores
    else:
