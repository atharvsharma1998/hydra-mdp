...
from navsim.common.enums import BoundingBoxIndex, LidarIndex, classproperty
...
class BoundingBox2DIndex(IntEnum):
    ...
    # NOTE: ``classproperty`` (not ``@classmethod @property``) so these resolve to
    # values on Python 3.8 (the unified env); the stacked-decorator form only
    # works on 3.9+ and otherwise returns a bound method.
    @classproperty
    def X(cls):
        return cls._X
